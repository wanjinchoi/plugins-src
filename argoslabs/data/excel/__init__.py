...
# --------
#  * [2024/05/14] Wanjin
#    openpyxl 3.1.5 버전의 경우 엑셀에 피벗테이블이 있을 경우 openpyxl_loadworkbook이 안됨 그래서 3.07로 낮추고 모두 처리후에 다시 최신버전으로 다운받는 코드를 넣음
#
#  * [2024/05/14] Wanjin
...
class Excel(object):
    ...
    def open(self, read_only=False, data_only=False, keep_vba=False):
        ...
        if self.extension == '.csv':
            ...
            print(f"Starting openpyxl version: {openpyxl.__version__}")
...
def do_excel(mcxt, argspec):
    ...
    try:
        # 기존에 설치된 openpyxl 패키지 제거
        print("Uninstalling openpyxl...")
        subprocess.check_call(
            [sys.executable, "-m", "pip", "uninstall", "-y", "openpyxl"])
    except Exception as e:
        print(f"Error occurred while uninstalling openpyxl: {e}")
    ...
    try:
        ...
        print(f"Error occurred while clearing pip cache: {e}")

    try:
        # openpyxl 3.0.7 버전 설치
        print("Installing openpyxl 3.0.7...")
        subprocess.check_call(
            [sys.executable, "-m", "pip", "install", "openpyxl==3.0.7"])
    except Exception as e:
        print(f"Error occurred while installing openpyxl 3.0.7: {e}")

    try:
        # 설치된 openpyxl 버전 확인
        openpyxl = importlib.import_module("openpyxl")
        print(f"Installed openpyxl version: {openpyxl.__version__}")
    except ModuleNotFoundError:
        print("openpyxl is not installed.")
    except Exception as e:
        print(f"Error occurred while checking openpyxl version: {e}")
    try:
        ...
        subprocess.check_call([sys.executable, '-m', 'pip', 'uninstall', '-y', 'openpyxl'])
